...
class WPS3B_Backup_Engine {
	...
	/** @var string Shared timestamp for all files in a single backup run */
	private $timestamp;

	/**
	 * @param array $exclude_paths Paths to exclude from file backup.
	 */
	public function __construct( $exclude_paths = array(), $timestamp = '' ) {
		$this->timestamp = ! empty( $timestamp ) ? $timestamp : gmdate( 'Y-m-d-His' );
	...
	private function get_filename( $suffix ) {
		return apply_filters( 'wps3b_backup_filename', $this->timestamp . '-' . $suffix, $suffix );
	}
