...
class WPS3B_S3_Client {
	...
	/** @var string Custom endpoint URL (for S3-compatible providers) */
	private $custom_endpoint;

	/** @var int Maximum retries for failed requests */
	const MAX_RETRIES = 3;

	/** @var int Multipart upload threshold in bytes (25MB — lower for shared hosting compatibility) */
	const MULTIPART_THRESHOLD = 26214400;

	/** @var int Multipart chunk size in bytes (10MB — smaller chunks for reliability) */
	const MULTIPART_CHUNK_SIZE = 10485760;
	 ...
	 */
	public function __construct( $access_key, $secret_key, $region, $bucket, $custom_endpoint = '' ) {
		$this->access_key      = $access_key;
		$this->secret_key      = $secret_key;
		$this->region          = $region;
		$this->bucket          = $bucket;
		$this->custom_endpoint = rtrim( $custom_endpoint, '/' );
	}
	...
	private function get_endpoint() {
		if ( ! empty( $this->custom_endpoint ) ) {
			// Path-style: https://endpoint/bucket or virtual-hosted: https://bucket.endpoint
			if ( false !== strpos( $this->custom_endpoint, $this->bucket ) ) {
				return $this->custom_endpoint;
			}
			return sprintf( 'https://%s.%s', $this->bucket, preg_replace( '#^https?://#', '', $this->custom_endpoint ) );
		}
	...
	private function get_host() {
		...
		return preg_replace( '#^https?://#', '', $endpoint );
	}
	...
	public function list_objects( $prefix = '' ) {
		...
		if ( preg_match( '/<Key>(.+?)<\/Key>/', $content, $m ) ) {
				...
				if ( preg_match( '/<StorageClass>(.+?)<\/StorageClass>/', $content, $m ) ) {
					$storage_class = $m[1];
				}
	...
	/**
	 * Copy an object within the same bucket (server-side, no download).
	 *
	 * @param string $source_key Source S3 object key.
	 * @param string $dest_key   Destination S3 object key.
	 * @return true|WP_Error
	 */
	public function copy_object( $source_key, $dest_key ) {
		$uri     = '/' . ltrim( $dest_key, '/' );
		$headers = $this->sign_request( 'PUT', $uri, '', array(
			'x-amz-copy-source' => '/' . $this->bucket . '/' . ltrim( $source_key, '/' ),
		), '' );
