...
class WPS3B_Plugin {
	...
	public static function init() {
		...
		add_action( 'wps3b_run_background_backup', array( 'WPS3B_Settings', 'run_background_backup' ) );
		add_action( 'wps3b_run_background_restore', array( 'WPS3B_Settings', 'run_background_restore' ) );
		...
		add_action( 'admin_init', array( __CLASS__, 'handle_restore_backup' ) );
		add_action( 'admin_init', array( __CLASS__, 'handle_confirm_restore' ) );
		add_action( 'admin_init', array( __CLASS__, 'maybe_schedule' ) );
		add_action( 'admin_init', array( __CLASS__, 'maybe_fix_permissions' ) );
	...
	/**
	 * Handle restore button — show confirmation page.
	 */
	public static function handle_restore_backup() {
		if ( ! isset( $_GET['wps3b_restore'] ) ) {
			return;
		}
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}
		check_admin_referer( 'wps3b_restore_backup' );

		$timestamp = sanitize_text_field( wp_unslash( $_GET['wps3b_restore'] ) );
		$manifest  = WPS3B_Restore::get_manifest( $timestamp );

		// Store manifest in transient for the confirmation page
		set_transient( 'wps3b_restore_manifest', array(
			'timestamp' => $timestamp,
			'manifest'  => $manifest,
		), 600 );

		wp_safe_redirect( admin_url( 'admin.php?page=wps3b_backups&wps3b_confirm_restore=1' ) );
		exit;
	}

	/**
	 * Handle restore confirmation — execute the restore.
	 */
	public static function handle_confirm_restore() {
		if ( ! isset( $_POST['wps3b_confirm_restore'] ) ) {
			return;
		}
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		...
		$result = WPS3B_Restore::run( $timestamp );

		if ( is_wp_error( $result ) ) {
			add_settings_error( 'wps3b_backups', 'wps3b_restore_failed',
				sprintf(
					/* translators: Error message */
					esc_html__( 'Restore failed: %s', 'wp-s3-backup' ),
					$result->get_error_message()
				),
				'error'
			);
		} else {
			add_settings_error( 'wps3b_backups', 'wps3b_restore_success',
				esc_html__( 'Restore completed successfully. Please go to Settings > Permalinks and click Save Changes.', 'wp-s3-backup' ),
				'success'
			);
		}
	}
	...
	public static function register_menus() {
		...
		if ( ! function_exists( 'wps3b_is_pro_active' ) || ! wps3b_is_pro_active() ) {
			add_submenu_page(
				'wps3b_settings',
				__( 'Upgrade to Pro', 'wp-s3-backup' ),
				'<span style="color:var(--bf-teal,#14B8A6);">' . __( 'Upgrade to Pro', 'wp-s3-backup' ) . '</span>',
				'manage_options',
				'wps3b_upgrade',
				array( 'WPS3B_Settings', 'render_upgrade_page' )
			);
		}
	}
	...
	public static function enqueue_assets( $hook ) {
		...
		wp_localize_script( 'wps3b-admin', 'wps3b_ajax', array(
			...
			'logs_nonce'   => wp_create_nonce( 'wps3b_refresh_logs' ),
			'backup_nonce'  => wp_create_nonce( 'wps3b_backup_step' ),
			'restore_nonce' => wp_create_nonce( 'wps3b_backup_step' ),
			'i18n'         => array(
				'testing'    => esc_html__( 'Testing connection...', 'wp-s3-backup' ),
				'test_btn'   => esc_html__( 'Test Connection', 'wp-s3-backup' ),
				'refreshing' => esc_html__( 'Refreshing...', 'wp-s3-backup' ),
				'updated'    => esc_html__( 'Updated', 'wp-s3-backup' ),
			),
		) );
	}
	...
	/**
	 * Check and fix file permissions if needed (runs once per day).
	 */
	public static function maybe_fix_permissions() {
		$last_check = get_option( 'wps3b_permissions_checked', 0 );
		if ( ( time() - $last_check ) < DAY_IN_SECONDS ) {
			return;
		}
		self::fix_permissions();
		update_option( 'wps3b_permissions_checked', time() );
	}

	/**
	 * Fix file permissions for the plugin directories.
	...
	public static function fix_permissions() {
		...
		$dirs = array( WPS3B_PLUGIN_DIR );

		// Also fix Pro plugin if it exists
		if ( defined( 'WPS3B_PRO_DIR' ) && is_dir( WPS3B_PRO_DIR ) ) {
			$dirs[] = WPS3B_PRO_DIR;
		}
		...
		foreach ( $dirs as $base_dir ) {
			try {
				...
				foreach ( $iterator as $item ) {
					$perms = fileperms( $item->getPathname() ) & 0777;
					if ( $item->isDir() && $perms !== 0755 ) {
						@chmod( $item->getPathname(), 0755 );
					} elseif ( $item->isFile() && $perms !== 0644 ) {
						@chmod( $item->getPathname(), 0644 );
					}
				}

				// Fix the base directory itself
				if ( ( fileperms( $base_dir ) & 0777 ) !== 0755 ) {
					@chmod( $base_dir, 0755 );
				}
			} catch ( Exception $e ) {
				// Silently fail — some hosts restrict chmod
			}
		}
	}
}
